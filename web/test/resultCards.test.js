...
// never appear, and nothing is shown twice.
//
// Run: cd web && node test/resultCards.test.js

import assert from 'node:assert/strict';
import { cardModel, FACT_KEYS } from '../src/lib/resultCards.js';
...
test('the fact strip follows FACT_KEYS order and folds zone1pct into zoning', () => {
  const m = cardModel(ROW);
...
test('a linked fact keeps its link (Assessment -> the MAO report)', () => {
