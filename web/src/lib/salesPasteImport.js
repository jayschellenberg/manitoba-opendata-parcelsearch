 ...
 *
 * Single-screen flow inside a <dialog>: a textarea + file picker, and
 * a Load button. Unlike the property-search Import-List modal there is
 * no column-mapping step — the sales schema is fixed, so parseSalesCsv
 * (in main.js) auto-detects the header aliases and the delimiter (tab
 * for spreadsheet/table copy-paste, comma for a real CSV). On Load the
 * driver hands { name, text } to the injected onSubmit callback, which
 * main.js wires straight into handleSalesUpload — the exact same
 * pipeline the dropzone uses, so paste and file uploads stay identical
 * downstream (including the Recent-uploads cache).
 */

/**
 * Wire up the modal. Returns { open, close } so callers can drive it
 * programmatically (the "Paste data…" link on the Sales tab).
 *
 * @param {Object} opts
...
/** Friendly label for a pasted (no-file) load — first non-empty line's
 *  leading chars + a timestamp, so pastes are distinguishable in the
 *  Recent-uploads dropdown. Mirrors the Import-List modal's helper. */
