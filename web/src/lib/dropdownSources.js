 ...
 *
 * The manifest is the only list that carries the number: the live
 * Roll_Entry probe asks for distinct Muni_Name_With_Typ and gets names
 * alone. That is fine — the number is decoration on the label, never the
 * option's value — so a boot where the manifest never lands just shows
 * bare names rather than holding the picker back for it.
 *
...
/**
 * How one municipality reads in the picker: "ARBORG (TOWN) - 300".
 *
 * Appended, not prefixed (Jason, 2026-09-14). The number is what you
 * quote to MAO and what a roll list is keyed on, so it belongs on screen —
 * but the name is what the list is sorted by and what the browser's own
 * select type-ahead matches, and both of those break if a number leads.
 *
 * Unknown number → the bare name. A picker that reads "ARBORG (TOWN) -
 * undefined" would be worse than one that stays quiet about the number.
 *
 * @param {string} name
...
export function muniOptionLabel(name, numbers) {
  ...
  return no == null ? String(name) : `${name} - ${no}`;
}
