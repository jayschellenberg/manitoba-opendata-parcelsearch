// MapLibre GL JS map setup with a self-hosted Protomaps (OpenStreetMap)
// vector basemap. No API key, no per-tile vendor quota: the whole street
// map is one .pmtiles archive on the mb-ortho R2 bucket, styled in the
// browser by @protomaps/basemaps. (CARTO's raster Voyager tiles, used
// until 2026-09, started watermarking unkeyed requests and are being
// retired.)
//
...
// `pmtiles extract --bbox` and uploaded to the mb-ortho R2 bucket next to
// the MLI ortho and the Assessment Parcels archive. Override for local
// work (e.g. the Protomaps demo planet archive) with
...
// The basemap's own layer stack, generated by @protomaps/basemaps for the
// "light" flavor: roads keep a real casing + colour hierarchy (the reason
// Voyager was chosen over Positron) on a pale ground the overlays read
// against. Two adjustments:
//   - ids are prefixed `pm-` so none can collide with an app layer
//     (Protomaps ships plain ids like `water` and `buildings`);
//   - "Noto Sans Medium" becomes "Open Sans Semibold", because the glyph
//     server the app already uses (demotiles.maplibre.org) has no Medium
//     stack and a missing stack silently drops every label that uses it.
//     text-font can be an expression, hence the JSON round-trip.
// Every layer starts visible; the basemap menu and setBasemapSatellite
// flip the whole set via STREETS_LAYER_IDS.
const STREETS_LAYERS = protomapsLayers('protomaps', namedFlavor('light'), { lang: 'en' })
  .map((layer) => {
    ...
    return {
      ...remapped,
      id: `pm-${layer.id}`,
      layout: { ...(remapped.layout || {}), visibility: 'visible' },
    };
  });
export const STREETS_LAYER_IDS = STREETS_LAYERS.map((l) => l.id);
...
const BASEMAP_STYLE = {
  ...
  // Road shields, one-way arrows and POI icons for the Protomaps streets
  // layers. Copied from protomaps/basemaps-assets (sprites/v4/light) into
  // web/public/basemap-sprites/ so the deploy is self-contained. MapLibre
  // wants an absolute sprite URL.
  ...
  sources: {
    ...
    // basemap clean (the Protomaps streets layers carry their own labels).
    'esri-reference': {
      type: 'raster',
      tiles: [
  ...
  layers: [
    // Protomaps streets layers are the default basemap. Satellite (Esri
...
export function initMap(container, { onFeatureClick, onPlacePick, getMunis } = {}) {
  ...
  const ready = new Promise((resolve) => {
    ...
    const setupMap = () => {
      ...
      // Hover reads, click pins — the same split the muni-parcels layer
      // uses. Reading a station's history is a scanning task (which of
      // these dots is the busy one?), and making that cost a click and a
      // dismiss per dot is the wrong trade. But the popup ends in a link
      // to the MHTIS web app, and you cannot reach a link inside a popup
      // that disappears when the cursor leaves the dot — so the pinned
      ...
      for (const layerId of ['traffic-circle', 'traffic-circle-town']) {
        onLayerClick(map, layerId, (e) => {
          const p = e.features?.[0]?.properties;
          if (!p) return;
          // Hand off cleanly: the pinned popup is about to say the same
          // thing, and two copies of it stacked at the cursor is noise.
          trafficHoverPopup.remove();
          trafficPopup.setLngLat(e.lngLat).setHTML(trafficHtml(p)).addTo(map);
        });
        // mousemove, not mouseenter: these dots cluster, and mouseenter
        // does not re-fire when the cursor crosses from one feature to
        // another within the same layer — the popup would keep showing
        // the first station you touched.
        map.on('mousemove', layerId, (e) => {
          if (map.getLayoutProperty(layerId, 'visibility') !== 'visible') return;
          // Stand down while the measuring tool owns the cursor, same as
          // the other hover popups.
          if (isMeasuring()) {
            trafficHoverPopup.remove();
            setHoverCursor('');
            return;
          }
          if (trafficPopup.isOpen()) { trafficHoverPopup.remove(); return; }
          const p = e.features?.[0]?.properties;
          if (!p) return;
          setHoverCursor('pointer');
          trafficHoverPopup
            .setLngLat(e.lngLat)
            .setHTML(trafficHtml(p, { link: false }))
            .addTo(map);
        });
 ...
 * overlays) or back to streets (Protomaps). Same layer-visibility swap
...
export function setBasemapSatellite(map, on) {
  ...
  const streetsVis = on ? 'none' : 'visible';
  ...
  for (const id of STREETS_LAYER_IDS) {
    if (map.getLayer(id)) map.setLayoutProperty(id, 'visibility', streetsVis);
  }
...
/**
 * @param {Object} p                station feature properties
 * @param {Object} [opts]
 * @param {boolean} [opts.link]     include the MHTIS web-app link. False for
 *   the hover popup, which vanishes the moment the cursor leaves the dot, so
 *   the link could never be reached from it.
 */
function trafficHtml(p, { link = true } = {}) {
  const lines = [];
...
class BasemapMenuControl {
  ...
  _set(state) {
    ...
    // road overlays on top; only Streets (Protomaps carries its own) and
    // the NRCan Transportation map opt out.
    const esriLabels = state !== 'streets' && state !== 'transportation';
    for (const id of STREETS_LAYER_IDS) m.setLayoutProperty(id, 'visibility', state === 'streets' ? 'visible' : 'none');
