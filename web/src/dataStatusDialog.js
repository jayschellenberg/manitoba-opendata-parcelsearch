...
import {
  ...
  fetchHistoricalIndex, fetchMascIndex, fetchWaterIndex, fetchFloodIndex,
} from './arcgis.js';
...
export function initDataStatusDialog() {
  ...
  async function load() {
    const [vintage, manifest, rollSnap, histIndex, mascIdx, waterIdx, floodIdx] = await Promise.all([
