...
- `Frontage_or_Area` from ROLL_ENTRY is sometimes acres, sometimes frontage-feet. When it parses as an area the table shows that assessor figure; when it's frontage-feet (or missing) the table falls back to computing acreage from the polygon. The **Acres Src** column records which was used. A nominal placeholder area on a large polygon (e.g. `0.01 ACRES` on crown/reserve land) is detected and overridden with the geometry figure.
