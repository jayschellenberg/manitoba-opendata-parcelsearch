...
| [MHTIS Traffic Flow 2023](https://www.gov.mb.ca/mti/traffic/counts.html) | `…/MHTIS_Traffic_Flow_2023_(new)/FeatureServer/0` | AADT polylines for the colour-coded Traffic overlay. Read `AADT_2023`, not `AADT` — the latter is a carried-forward prior estimate the service still ships |
...
- **Dropdown lists + auxiliary overlays** — cached in IndexedDB (localStorage fallback) under the `mbpsCache.` namespace. The municipality / zone-category / dev-plan-category lists are served **stale-while-revalidate**: a cached list younger than 7 days is used as-is; one older than 7 days (up to 180) is still shown instantly while a single background refresh rewrites it; only a never-cached or >180-day list waits on the live query. The municipality picker also paints from the first list to arrive (cached live list or snapshot-manifest names) rather than waiting on the zoning and record-count probes, and a live muni answer under 150 names (a partial provincial republish) is shown but never cached. Most other lists and live overlays use a 7-day TTL; stable generated/reference overlays such as MASC, MASC Risk Areas, municipal boundaries, river lots, and the section grid use a 30-day TTL. Quota recovery evicts older namespaced entries before failing. Clear button wipes the namespace.
...
- `MUNI_WEBSITES` and `PD_WEBSITES` (in [main.js](web/src/main.js)) are hand-curated from the province's official Municipal and Planning District contact directories, and nothing refreshes them. A muni with no entry renders as "Muni N/A" — adding an entry promotes it to a working button. **A missing entry is better than a wrong one:** an absent key degrades honestly, while a rotted URL lights the button and opens a browser error or a domain squatter, which reads as this app's bug. A full check on 2026-08-05 found 15 of 153 URLs dead (6.6% of munis, 26% of planning districts) — repointed where a replacement existed, removed otherwise. Half the failures returned HTTP 200, and many municipalities legitimately brand off-name, so an automated checker would flag more good entries than bad; re-derive from the provincial directory after the October 2026 municipal elections instead.
